...
import { storageService } from '.../services/asyn-storage.service.js'
...
function query(filterBy = {}) {
  return utilService.loadFromStorage(MAIL_KEY).then((mail) => {
    const { from, to, subject, hasWords, dontHaveWords, dateFrom, dateTo, searchCategory, attachments } = filterBy
    ...
    if (dateFrom || dateTo) {
      ...
      mail = mail.filter((mail) => fromDate <= mail.date && mail.date <= toDate)
    }
    if (searchCategory) {
      mail = mail.filter((mail) => mail.category === searchCategory)
    }
    if (attachments) {
      mail = mail.filter((mail) => mail.attachments === true)
    }
    return mail
  })
}
...
function get(mailId) {
  return stortagService.getMail(MAIL_KEY, mailId).then((mail) => {
...
function remove(mailId) {
  return stortagService.remove(MAIL_KEY, mailId)
}
...
function _setNextPrevMail(mail) {
  return stortagService.query(MAIL_KEY).then((mails) => {
